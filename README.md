...
`biu` is a zero-config, high-performance bundler for HTML files with TypeScript/JavaScript, powered by [Bun](https://bun.sh/). It is designed to handle module splitting with custom import suffixes and provides built-in minification for HTML, CSS, and JS.
...
- **Minification**: Built-in minification for HTML, CSS, and TypeScript/JavaScript.
- **Watch Mode**: Live rebuilds on file changes.
- **Fast**: Built on the lightning-fast Bun runtime.

## Installation & Compilation

Since `biu` is built with Bun, you can compile it into a single executable binary for portability.
...
- **Independent Module**: By default, linked `.ts` or `.js` files are treated as independent modules.
